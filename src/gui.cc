...
void GUI::drawGauge() {
	...
	// draws background
	if (!pix_stick) {
		pix_stick = gdk_pixmap_create_from_xpm_d(gauge_area->window, NULL, 
	...
	gdk_draw_line(w, gc, 80, 99, 80 + (int)rint(90.0*sin(gauge->getPosition()
			*M_PI/(1.5*max))), 99 - (int)rint(90.0*cos(gauge->getPosition()
			*M_PI/(1.5*max))));
...
# define PLOT_GAIN  8
	...
	if (core->freq != 0.0) {
		...
		int i = (int) rint(core->freq*conf->FFT_SIZE*conf->OVERSAMPLING
				/conf->SAMPLE_RATE);
		int j = (core->X[i] > 1.0) ? (int) (64 - PLOT_GAIN*log10(core->X[i])) : 64; // dB.
