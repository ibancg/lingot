...
void lingot_core_destroy(LingotCore* core)
  {
    ...
    free(core->temporal_window_buffer);
...
# endif
    ...
    if (core->conf->temporal_buffer_size > core->conf->read_buffer_size)
      memcpy(core->temporal_window_buffer,
          &core->temporal_window_buffer[core->conf->read_buffer_size],
          (core->conf->temporal_buffer_size - core->conf->read_buffer_size)
              *sizeof(FLT));
    ...
    if (core->conf->oversampling > 1)
      lingot_core_decimate(
          ...
          &core->temporal_window_buffer[core->conf->temporal_buffer_size - core->conf->read_buffer_size]);
    else
      memcpy(
          &core->temporal_window_buffer[core->conf->temporal_buffer_size - core->conf->read_buffer_size],
...
# ifdef LIB_FFTW
    ...
    fftw_in[i].re =
    core->temporal_window_buffer[core->conf->temporal_buffer_size - core->conf->fft_size + i];
...
# else
    ...
    // transformation.
    lingot_fft_fft(
        &core->temporal_window_buffer[core->conf->temporal_buffer_size - core->conf->fft_size],
        core->fft_out, core->conf->fft_size);
        ...
        if (k == 0)
          {
            lingot_fft_spd(
                &core->temporal_window_buffer[core->conf->temporal_buffer_size - core->conf->fft_size],
                core->conf->fft_size, w + d_w, d_w, &core->spd_dft[1],
                core->conf->dft_size- 2);
          ...
          lingot_fft_spd(
              &core->temporal_window_buffer[core->conf->temporal_buffer_size - core->conf->fft_size],
              core->conf->fft_size, w, d_w, core->spd_dft, core->conf->dft_size);
